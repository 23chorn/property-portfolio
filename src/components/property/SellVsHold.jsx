        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="text-right py-2 text-xs text-text-muted uppercase">Sell & Invest</th>
              <th className="text-right py-2 text-xs text-text-muted uppercase">Hold</th>
              <th className="text-right py-2 text-xs text-text-muted uppercase">Advantage</th>
            </tr>
          </thead>
          <tbody>
            {sensitivityData.map((row, i) => (
              <tr key={i} className={`border-b border-border/30 ${i % 2 === 0 ? 'bg-bg-elevated/30' : ''}`}>
                <td className="py-2.5 text-text-secondary">{row.growthRate}</td>
                <td className="py-2.5 text-right font-mono text-accent-blue">{formatCurrency(row.sellPath)}</td>
                <td className="py-2.5 text-right font-mono text-accent-green">{formatCurrency(row.holdPath)}</td>
                <td className={`py-2.5 text-right font-mono font-medium ${row.advantage >= 0 ? 'text-accent-green' : 'text-accent-red'}`}>
