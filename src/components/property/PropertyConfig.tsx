  ...
  return (
    <div className="bg-stone-800 border border-stone-700 rounded-xl p-4 sm:p-6">
      <div className="flex justify-between items-center mb-5">
        <h3 className="text-sm font-semibold text-stone-100 uppercase tracking-wider">{title}</h3>
        {onReset && (
          <button
            onClick={onReset}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <ConfigSection title="Property Details" onReset={() => resetSection('meta')}>
          <div className="space-y-3">
            <div>
              <label className="text-sm text-stone-400">Property Name</label>
              <input
                type="text"
                value={meta.name}
                onChange={(e) => updateField('meta', 'name', e.target.value)}
                className="w-full mt-1 bg-stone-700 border border-stone-700 rounded px-3 py-2 text-sm text-stone-100 focus:outline-none focus:border-rose-500"
              />
            </div>
            <div>
              <label className="text-sm text-stone-400">Address</label>
              <input
                type="text"
                value={meta.address}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-sm text-stone-400">EPC Rating</label>
                <select
        ...
        <ConfigSection title="Mortgage" onReset={() => resetSection('mortgage')}>
          <div className="space-y-3">
            <div>
              <label className="text-sm text-stone-400">Mortgage Type</label>
              <select
                value={mortgage.type}
        ...
        <ConfigSection title="Running Costs" onReset={() => resetSection('costs')}>
          ...
          <div className="mt-4">
            <p className="text-xs text-stone-500 uppercase tracking-wider mb-2">Additional Costs</p>
            {costs.additionalCosts.map((cost, i) => (
              <div key={i} className="flex items-center gap-2 mb-2">
                <span className="flex-1 text-sm text-stone-400 truncate">{cost.label}</span>
                <span className="font-mono text-sm text-stone-100">{formatCurrency(cost.amount)}</span>
                <span className="text-xs text-stone-500">{cost.year}</span>
                <button onClick={() => removeAdditionalCost(i)} className="text-rose-400 text-xs hover:text-rose-400/80">x</button>
              </div>
            ))}
            <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-2 mt-3">
              <input
                ...
                className="flex-1 bg-stone-700 border border-stone-700 rounded px-2 py-1.5 text-xs text-stone-100 focus:outline-none focus:border-rose-500"
              />
              <div className="flex gap-2">
                <input
                  ...
                  className="w-24 bg-stone-700 border border-stone-700 rounded px-2 py-1.5 text-xs text-stone-100 focus:outline-none focus:border-rose-500"
                />
                <input
                  type="number"
                  placeholder="Year"
                  ...
                  className="w-20 bg-stone-700 border border-stone-700 rounded px-2 py-1.5 text-xs text-stone-100 focus:outline-none focus:border-rose-500"
                />
                <button onClick={addAdditionalCost} className="text-xs bg-rose-600/10 text-rose-400 px-3 py-1.5 rounded hover:bg-rose-600/20 transition-colors whitespace-nowrap">
                  Add
                </button>
              </div>
            </div>
        ...
        <ConfigSection title="Tax Position" onReset={() => resetSection('tax')}>
          <div className="flex items-center gap-3 mb-3">
            <label className="text-sm text-stone-400">UK Citizen</label>
            <button
              onClick={() => updateField('tax', 'ownerIsUKCitizen', !tax.ownerIsUKCitizen)}
              className={`w-10 h-5 rounded-full transition-colors ${tax.ownerIsUKCitizen ? 'bg-emerald-500' : 'bg-stone-700'}`}
            >
          ...
          <div className="flex items-center gap-3 mb-3">
            <label className="text-sm text-stone-400">Claims Personal Allowance</label>
            <button
              onClick={() => updateField('tax', 'claimsPersonalAllowance', !tax.claimsPersonalAllowance)}
              className={`w-10 h-5 rounded-full transition-colors ${tax.claimsPersonalAllowance ? 'bg-emerald-500' : 'bg-stone-700'}`}
            >
          ...
          <div className="flex items-center gap-3 mb-3">
            <label className="text-sm text-stone-400">NRLS Registered</label>
            <button
              onClick={() => updateField('tax', 'nrlsRegistered', !tax.nrlsRegistered)}
              className={`w-10 h-5 rounded-full transition-colors ${tax.nrlsRegistered ? 'bg-emerald-500' : 'bg-stone-700'}`}
            >
              <span className={`block w-4 h-4 rounded-full bg-white transition-transform ${tax.nrlsRegistered ? 'translate-x-5' : 'translate-x-0.5'}`} />
            </button>
          </div>
          <div>
            <label className="text-sm text-stone-400">Residence Country</label>
            <input
              type="text"
