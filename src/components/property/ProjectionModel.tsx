  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="bg-stone-800 border border-stone-700 rounded-xl p-4 sm:p-6">
        <h3 className="text-sm font-semibold text-stone-100 uppercase tracking-wider mb-4">Value & Equity Trajectory</h3>
        <ResponsiveContainer width="100%" height={220} className="sm:!h-[300px]">
          <ComposedChart data={chartData} margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey="equity" name="Equity" fill="#34d399" fillOpacity={0.1} stroke="#34d399" strokeWidth={2} activeDot={{ stroke: '#f5f5f4', strokeWidth: 1.5, r: 5 }} />
            <Line type="monotone" dataKey="cumulativeCashflow" name="Cumulative Cashflow" stroke="#fbbf24" strokeWidth={2} dot={false} activeDot={{ stroke: '#f5f5f4', strokeWidth: 1.5, r: 5 }} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>

      <div className="bg-stone-800 border border-stone-700 rounded-xl p-4 sm:p-6">
        <h3 className="text-sm font-semibold text-stone-100 uppercase tracking-wider mb-4">Monthly Net Cashflow by Year</h3>
          ...
          <BarChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            ...
            <Bar dataKey="monthlyNet" name="Monthly Net" radius={[3, 3, 0, 0]} activeBar={{ stroke: '#f5f5f4', strokeWidth: 1.5 }}>
              {chartData.map((entry, i) => (
                <Cell key={i} fill={entry.monthlyNet >= 0 ? '#34d399' : '#fb7185'} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      ...
      <div className="bg-stone-800 border border-stone-700 rounded-xl p-4 sm:p-6 overflow-x-auto">
        <h3 className="text-sm font-semibold text-stone-100 uppercase tracking-wider mb-4">Year-by-Year Breakdown</h3>
        <table className="w-full text-xs min-w-[700px]">
          <thead>
            <tr className="border-b border-stone-700">
              <th className="text-left py-2 text-stone-500">Year</th>
              <th className="text-right py-2 text-stone-500">Rent/mo</th>
              <th className="text-right py-2 text-stone-500">Annual Rent</th>
              <th className="text-right py-2 text-stone-500">Costs</th>
              <th className="text-right py-2 text-stone-500">Mortgage</th>
              <th className="text-right py-2 text-stone-500">Tax</th>
              <th className="text-right py-2 text-stone-500">Net/mo</th>
              <th className="text-right py-2 text-stone-500">Cumulative</th>
              <th className="text-right py-2 text-stone-500">Value</th>
              <th className="text-right py-2 text-stone-500">LTV</th>
            </tr>
          </thead>
          <tbody>
            {projection.map((y) => (
              <tr key={y.year} className={`border-b border-stone-700/30 ${y.phase === 2 ? 'bg-purple-400/5' : y.year % 2 === 0 ? 'bg-stone-700/30' : ''}`}>
                <td className="py-2 text-stone-400">{y.year}</td>
                <td className="py-2 text-right font-mono text-stone-100">{formatCurrency(y.monthlyRent)}</td>
                <td className="py-2 text-right font-mono text-stone-100">{formatCurrency(y.annualRent)}</td>
                <td className="py-2 text-right font-mono text-rose-400">{formatCurrency(y.totalCosts)}</td>
                <td className="py-2 text-right font-mono text-stone-100">{formatCurrency(y.mortgagePayment)}</td>
                <td className="py-2 text-right font-mono text-rose-400">{formatCurrency(y.tax)}</td>
                <td className={`py-2 text-right font-mono ${y.monthlyNet >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{formatCurrency(y.monthlyNet)}</td>
                <td className={`py-2 text-right font-mono ${y.cumulativeCashflow >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{formatCurrency(y.cumulativeCashflow)}</td>
                <td className="py-2 text-right font-mono text-stone-100">{formatCurrency(y.propertyValue)}</td>
                <td className="py-2 text-right font-mono text-stone-400">{y.phase === 2 ? '-' : `${y.ltv.toFixed(1)}%`}</td>
              </tr>
            ))}
          </tbody>
        </table>
